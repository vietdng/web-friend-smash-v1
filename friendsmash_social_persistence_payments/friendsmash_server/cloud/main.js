...
var APP_ACCESS_TOKEN = '1393332784218683|inA7GuW0_n1upgxm-6DsIALsFCE';

var Payment = Parse.Object.extend('Payment');

Parse.Cloud.define('fulfillPaymentA', function(request, response) {
  Parse.Cloud.httpRequest({
    ...
    if( !paymentJSON.actions 
      ...
      response.error('Payment not completed!');
    } else {
      response.success('Payment completed!');  
    }
  }, function(httpError) {
    response.error('Graph API error: ' + httpError.data.error.message );  
  });
});

Parse.Cloud.define('fulfillPaymentB', function(request, response) {
  Parse.Cloud.httpRequest({
    url: 'https://graph.facebook.com/'+request.params.payment_id+'?access_token=' + APP_ACCESS_TOKEN
  }).then( function(httpResponse) {
    var paymentJSON = httpResponse.data;
    ...
    request.user.increment('coins', paymentJSON.items[0].quantity);
...
Parse.Cloud.define('fulfillPaymentC', function(request, response) {
  Parse.Cloud.httpRequest({
    url: 'https://graph.facebook.com/'+request.params.payment_id+'?access_token=' + APP_ACCESS_TOKEN
  }).then( function(httpResponse) {
    var paymentJSON = httpResponse.data;
